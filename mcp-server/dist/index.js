...
function ghHeaders() {
    const headers = {
        "User-Agent": "awesome-copilot-mcp-server",
...
async function fetchFullTree() {
    ...
    if (!branchResp.ok)
        throw new Error(`Failed to read branch info: ${branchResp.status}`);
    ...
    if (!treeResp.ok)
        throw new Error(`Failed to read repo tree: ${treeResp.status}`);
    const tree = await treeResp.json();
    await delay(150);
    return (tree.tree || []);
}
async function fetchFileContentRaw(url) {
    await checkRateLimit();
...
function hasCorrectExtension(filename, type) {
    if (type === "instruction")
...
function extractTitle(filename, type) {
    let title = filename;
    if (type === "instruction")
        title = title.replace(".instructions.md", "");
    if (type === "prompt")
        title = title.replace(".prompt.md", "");
    if (type === "chatmode")
        title = title.replace(".chatmode.md", "");
    return title.replace(/[-_]/g, " ").replace(/\b\w/g, c => c.toUpperCase());
}
function generateId(filename, type) {
...
async function buildIndex(forceRefresh = false) {
    if (!forceRefresh && cachedItems && isCacheValid())
        return cachedItems;
    ...
    const items = limited.map(({ path, type }) => {
...
function keywordSearch(items, query) {
...
async function expandIndexByKeywords(query) {
    ...
    for (const entry of tree) {
        ...
        const filename = entry.path.split("/").pop() || entry.path;
        let type;
        if (lowerPath.startsWith("instructions/") && hasCorrectExtension(filename, "instruction"))
            type = "instruction";
        else if (lowerPath.startsWith("prompts/") && hasCorrectExtension(filename, "prompt"))
            type = "prompt";
        else if (lowerPath.startsWith("chatmodes/") && hasCorrectExtension(filename, "chatmode"))
            type = "chatmode";
        if (!type)
            continue;
    ...
    const newItems = limited.map(({ path, type }) => {
        const filename = path.split("/").pop() || path;
        return {
            id: generateId(filename, type),
            type,
...
const mcp = new McpServer({ name: "awesome-copilot-mcp-server", version: "0.1.0" });
...
mcp.tool("search", {
    ...
    if (results.length === 0)
        results = keywordSearch(await expandIndexByKeywords(String(query || "")), String(query || ""));
...
mcp.tool("preview", {
    ...
    if (!item)
        throw new Error("Item not found in index; run build_index first");
...
mcp.tool("set_repo", {
    ...
    CONTENT_REPO = repo || CONTENT_REPO;
    if (branch)
        CONTENT_BRANCH = String(branch);
    if (typeof maxItems === "number" && maxItems >= 1)
        MAX_ITEMS = Math.max(1, Math.floor(maxItems));
    if (typeof cacheTtlHours === "number" && cacheTtlHours >= 1)
        CACHE_TTL_HOURS = Math.max(1, Math.floor(cacheTtlHours));
    cachedItems = null;
    cachedAt = null;
